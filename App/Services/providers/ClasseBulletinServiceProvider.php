...
class ClasseBulletinServiceProvider
{
  ...
  if($tab) foreach ($this->bulletins2 as $bulletin) {
    ...
    foreach ($this->getEleves() as $eleve) {
        $bulletinServiceProvider=new bulletinServiceProvider($this->anneeScolaireService,$eleve->matricule);
        $this->bulletins2[$eleve->matricule]=$bulletinServiceProvider->getBulletin2($tab);
    }
  ...
  if($tab) foreach ($this->bulletins1 as $bulletin) {
    ...
    foreach ($this->getEleves() as $eleve) {
        $bulletinServiceProvider=new bulletinServiceProvider($this->anneeScolaireService,$eleve->matricule);
        $this->bulletins1[$eleve->matricule]=$bulletinServiceProvider->getBulletin1($tab);
    }
  ...
  public function getStatistiques($type,$tab=[]):ClasseStatistiqueProviderEntity{
   ...
   $data= match ($type) {
    ...
    $statistiques->admis=count(array_filter($data, fn($bulletin) => $bulletin->getDecision()==BulletinProvider::DECISION_ADMIS));
    $statistiques->nonAdmis=count(array_filter($data, fn($bulletin) => $bulletin->getDecision()==BulletinProvider::DECISION_NON_ADMIS));
    $statistiques->mentionTresFaible=count(array_filter($data, fn($bulletin) => $bulletin->getMension()==BulletinProvider::MENTION_TRES_FAIBLE));
    $statistiques->mentionFaible=count(array_filter($data, fn($bulletin) => $bulletin->getMension()==BulletinProvider::MENTION_FAIBLE));
    $statistiques->mentionPassable=count(array_filter($data, fn($bulletin) => $bulletin->getMension()==BulletinProvider::MENTION_PASSABLE));
    $statistiques->mentionBien=count(array_filter($data, fn($bulletin) => $bulletin->getMension()==BulletinProvider::MENTION_BIEN));
    $statistiques->mentionAssezBien=count(array_filter($data, fn($bulletin) => $bulletin->getMension()==BulletinProvider::MENTION_ASSEZ_BIEN));
    $statistiques->mentionTresBien=count(array_filter($data, fn($bulletin) => $bulletin->getMension()==BulletinProvider::MENTION_TRES_BIEN));
    return $statistiques;
 }
}