...
class CorrespondantApiController extends Controller 
{
    ...
    public function id($idCorrespondant): void
    {
        ...
        if (!$data) {
            $this->response([
                'response' => "ko",
                'message' => "Le correspondant n'a pas été trouvé",
                'status' => 0
            ]);
            return;
        }
        ...
        if ($res) {
            ...
            $this->response(
                [
                    "data" => $data,
                    'response' => "ok",
                    'message' => "Le correspondant a été ajouté",
                    'status' => 1
                ]
            );
        } else
            $this->response([
                'response' => "ko",
                'message' => "Le correspondant n'a pas été ajouté",
                'status' => 0
            ]);
    ...
    public function delete($idCorrespondant): void
    {
        ...
        $this->response([
            "data" => $data,
            'response' => "ok",
            'message' => "Le correspondant a été supprimé",
            'status' => 200
        ]);
        } catch (\PDOException $th) {
            $this->response([
        ...
        if ($res) {
            ...
            $this->response(
                [
                    "data" => $data,
                    'response' => "ok",
                    'message' => "Le correspondant a été mis à jour",
                    'status' => 1
                ]
            );
        } else
            $this->response([
                'response' => "ko",
                'message' => "Le correspondant n'a pas été mis à jour",
                'status' => 0,
                'data' => []
            ]);
        } catch(\PDOException $th) {
