...
import CorrespondantForm from "./correspondant_form.js";
/**
...
function deleteCorrespondant(id) {
    if (confirm("Êtes-vous sûr de vouloir supprimer ce correspondant ?")) {
        fetchJson("?p=api/correspondant/delete/" + id).then((data) => {
            if (data?.status ?? 0) {
...
function editCorrespondant(id) {
    let dialog = new CorrespondantForm(document.querySelector(".dialog"), id);
...
document.getElementById("add")?.addEventListener("click", function (e) {
    let dialog = new CorrespondantForm(document.querySelector(".dialog"));
    dialog.reset();
...
if (data.length == 0) {
    let url = "?p=api/correspondant/liste";
...
trs.forEach(tr => {
    ...
    window.location.assign("?p=Correspondant/profil/" + tr.dataset.id)
    })
});
...
class TableData {
   ...
   async #render(data) {
       ...
       /*  data.forEach((element) => {
        ...
        let url = "?p=api/correspondant/htmlListe";
        ...
        tbody.querySelectorAll('tr').forEach(element => {
            element?.addEventListener("dblclick", async (e) => {
                let id = element.dataset.id;
                window.location.assign('?p=Correspondant/profil/' + id);
            });
        });
    }
